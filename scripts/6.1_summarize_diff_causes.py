...
if __name__ == "__main__":
    ...
    unexplained_path = sys.argv[2] if len(sys.argv) > 2 else "../outputs/unexplained_differences.tsv"

    BASE_OUTPUT_FOLDER="../outputs"
    benchmark1_summary_path = f"{BASE_OUTPUT_FOLDER}/benchmark1_summary_combined-{MEAN}.tsv"
    out_path = f"{BASE_OUTPUT_FOLDER}/benchmark1_difference_explanation-{MEAN}.tsv"

    # loop over the benchmark1_summary.tsv file
    total_sequences = 0
    total_diff_1_2 = 0
    ...
    species_checked = 0
    with open(benchmark1_summary_path, "r") as inf, open(unexplained_path, "w") as unexplained_outf:
        header = inf.readline()
        for line in inf:
            line = line.strip()
            items = line.split("\t")
            species_id = items[0]
            taxonomic_group = items[1]

            # used for testing
            # if taxonomic_group not in ["protozoa"]:
            #     continue

            num_sequences = int(items[2])
                            ...
                            if ramp1_fixed_truncated == ramp2_truncated:
                                ...
                                if len(diff_headers_truncated) > 0:
                                    for header in diff_headers_truncated:
                                        ...
                                        unexplained_outf.write(f"{species_id}\t{taxonomic_group}\t{header}\t{ramp1_fixed_truncated_path}\t{ramp2_truncated_path}\n")
                                        unexplained_outf.flush()
                                        total_diff_1_2_not_explained += 1

                        else:
                            # if this error is printed, the results will not be accurat- fix the issue and rerun
                            print(f"ERROR: Truncated files missing for {species_id} | {taxonomic_group}")
            ...
            species_checked += 1
            if species_checked % 100 == 0:
                print(f"Checked {species_checked} species...")
    print(f"Checked {species_checked} species total.")
