...
from math import exp, log, sqrt, ceil, trunc
import gzip
import csv
import argparse
import sys
...
VERSION = "1.0 timed fixed truncated"
...
def findSpeeds(elem):
    ...
    """
    csvLines = []
    for i in range(len(seqToSpeed[elem[0]])-ribosomeWindowLength+1):
        riboSmoothedSpeed = truncate(middleFunc[args.middle](seqToSpeed[elem[0]][i:i+ribosomeWindowLength]), 13)
...
def isolateRampHmean(record):
    ...
    mean = hmean(speeds)
    windowMeans = []
    for z in range(len(speeds)-ribosomeWindowLength+1):
        windowMeans.append(truncate(middleFunc[args.middle](speeds[z:z+ribosomeWindowLength]), 13))
...
def truncate(num, decs=0):
    """
    Truncates a number to 'decs' decimal places.
    """
    factor = 10**decs
...
if __name__ == '__main__':
    ...
    if args.verbose:
        ...
        sys.stderr.write("\nTotal valid sequences: " +str(len(seqArray)) + '\n\n')
    if len(seqArray) ==0:
    ...
    if args.verbose:
        sys.stderr.write(f"took: {time.time() - start}\n")
        if args.rscu:
            sys.stderr.write(f"\tReading RSCU file took: {rscuTime}\n")
    if args.verbose:
    ...
    if args.determine_cutoff:
        ...
        if args.verbose:
            sys.stderr.write(f"took: {time.time() - pool_start}\n")
