...
def makeArgParser():
    ...
    parser.add_argument("-y", "--scores", default = None, type=str, help="(output) tsv file to write ramp strength and robustness scores.")
    ...
    parser.add_argument("-g", "--prevalidate", action="store_true", help="flag to truncate sequences at early stop codons before testing if sequences are valid")
...
def getSlidingMean(middle, a, ribosomeWindowLength):
    ...
    windowMeans = trunc(windowMeans, 13)
    return windowMeans

def trunc(a, decs=0):
    """
    Truncates all values in a numpy array to 'decs' decimal places.
    """
    factor = 10**decs
    return np.trunc(a*factor) / factor
...
if __name__ == "__main__":
    ...
    if args.scores:
        ...
        if args.verbose:
            ...
            start = time.time()
        outputRampScores(rampScores, args.scores)
        if args.verbose:
            sys.stderr.write(f"took: {time.time() - start}\n")
