...
if [ "$SCORES" = "True" ] && [ "$VERSION" = "2.0" ]; then
    scriptFileName=$BASE_SCRIPT_FOLDER/${VERSION}-$MEAN-${TAXON_GROUP}-scores.sh
else
    scriptFileName=$BASE_SCRIPT_FOLDER/${VERSION}-$MEAN-${TAXON_GROUP}.sh
fi
...
if [ "$SCORES" = "True" ] && [ "$VERSION" = "2.0" ]; then
    echo "#SBATCH -J \"run-${VERSION}-${MEAN}-${TAXON_GROUP}-scores\"" >> $scriptFileName
else
    echo "#SBATCH -J \"run-${VERSION}-${MEAN}-${TAXON_GROUP}\"" >> $scriptFileName
fi
...
echo -e "\tcds=\${dir##*/}" >> $scriptFileName
echo -e "\tout_path=\"$BASE_OUT_FOLDER/\$cds-hmean-w9-ramps.fa\"" >> $scriptFileName
# if the output file(s) don't exist, run ExtRamp on the input
if [ "$SCORES" = "True" ] && [ "$VERSION" = "2.0" ]; then
    echo -e "\tscores_out_path=\"$BASE_OUT_FOLDER/\$cds-hmean-w9-scores.tsv\"" >> $scriptFileName
    echo -e "\tif [ ! -f \$out_path ] || [ ! -f \$scores_out_path ]; then" >> $scriptFileName
else
    echo -e "\tif [ ! -f \$out_path ]; then" >> $scriptFileName
fi
