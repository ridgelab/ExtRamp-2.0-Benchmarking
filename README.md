...
The results of each of the mean benchmarks are read from the slurm files using the [8_get_benchmark2_data.py](./scripts/8_get_benchmark2_data.py) script and the outputs are written to the time_mem directory in the outputs directory: `../outputs/time_mem/benchmark2_results_{MEAN}.tsv`. Each line represents the results of one run of ExtRamp, either v1 or v2.
```
...
```
[8_process_benchmark2_data.py](./scripts/8_process_benchmark2_data.py) creates two plots and summary stats for each mean function. The plots compare time and memory to the number of sequences between v1 and v2. The summary file summarizes the file generated by [8_get_benchmark2_data.py](./scripts/8_get_benchmark2_data.py) so that each line represents a species and times and memory usage are averaged across all runs. All three outputs for each are written to the time_mem outputs directory. Here are the outputs for hmean: [Hmean Time Plot](./outputs/time_mem/num_seq_to_time-hmean.png), [Hmean Memory Plot](./outputs/time_mem/num_seq_to_mem-hmean.png), [Hmean Summary Stats](./outputs/time_mem/benchmark2_summary_hmean.tsv)
```
